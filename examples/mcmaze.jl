...
file_path = "/Users/ahmed.elgazzar/code/Rhythms/Datasets/NLB/mc_maze.npy" # change this to the path of the dataset
...
train_loader = DataLoader((Y_train, Y_train), batchsize=64, shuffle=true)
...
sensealg=TrackerAdjoint())#InterpolatingAdjoint(; autojacvec= ZygoteVJP()))
#dynamics = ODE(drift, Tsit5(), dt=ts[2]-ts[1], saveat=ts)
...
model, θ_best = train(model, p, st, train_loader, val_loader, 10, 1)
