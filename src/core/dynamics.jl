...
"""
struct ODE <: AbstractExplicitContainerLayer{((:vector_field,))}
    vector_field
    solver
    kwargs
end
...
function (de::ODE)(x₀::AbstractArray, u::Union{Nothing, AbstractArray}, ts::AbstractArray, p::ComponentArray, st::NamedTuple)
    u_cont(t) = CRC.@ignore_derivatives interp!(ts, u, t)
    dxdt(x, p, t) = de.vector_field((x, u_cont(t)), p, st)[1]
    prob = ODEProblem{false}(dxdt, x₀, (ts[1], ts[end]), p)
...
"""
function sample_dynamics(de::ODE, init_map, x̂₀, u, ts, p, st, n_samples, dev)
    u_cont(t) = CRC.@ignore_derivatives interp!(ts, u, t)
    x₀ = sample_rp(x̂₀)
    dxdt(x, p, t) = de.vector_field((x, u_cont(t)), p, st.dynamics)[1]
    ff = ODEFunction{false}(dxdt; tgrad = basic_tgrad)
    prob = ODEProblem{false}(ff, x₀, (ts[1], ts[end]), p.dynamics)

    function prob_func(prob, i, repeat)
        remake(prob, u0=init_map(sample_rp(x̂₀), p.init_map, st.init_map)[1])
    end
    ensemble_prob = EnsembleProblem(prob, prob_func = prob_func)
    ensemble_sol = solve(ensemble_prob, de.solver, EnsembleThreads(); trajectories=n_samples, de.kwargs...) |> dev
...
function (de::SDE)(x::AbstractArray, u::Union{Nothing, AbstractArray}, c::AbstractArray, ts::AbstractArray, p::ComponentVector, st::NamedTuple)
    ...
    function μ_prior(x, p, t)
        ...
        #return dxdt_u(de.drift, x, u_t, t, p, st.drift)[1]
    end
...
"""
function sample_dynamics(de::SDE, init_map, x̂₀, u, ts, p, st, n_samples, dev)
    tspan = (ts[1], ts[end])
    u_cont(t) = interp!(ts, u, t)
    x₀ = init_map(sample_rp(x̂₀), p.init_map, st.init_map)[1]
